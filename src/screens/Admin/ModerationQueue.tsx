...
export default function ModerationQueue() {
  ...
  async function updateStatus(id: string, status: string) {
    ...
    else setReports(rs => rs.map(r => r.id === id ? { ...r, status } : r))
  }

  async function hidePost(postId: string) { 
    Alert.alert('Confirm', 'Are you sure you want to hide this post?', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Hide', style: 'destructive', onPress: async () => {
        const { error } = await supabase.from('channel_posts').update({ deleted_at: new Date().toISOString() }).eq('id', postId)
        if (error) Alert.alert('Error', error.message)
        else {
          Alert.alert('Success', 'Post hidden.')
  ...
  async function lockChannel(channelId: string, hours: number) { 
    Alert.alert('Confirm', `Lock channel for ${hours} hours?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Lock', style: 'destructive', onPress: async () => {
        const until = new Date()
        until.setHours(until.getHours()+hours)
        const { error } = await supabase.from('channel_locks').upsert({ channel_id: channelId, until: until.toISOString() })
        if (error) Alert.alert('Error', error.message)
        else {
          Alert.alert('Success', 'Channel locked.')
